...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		menuscripts.executeAllFunc("startIntro", []);
		ClientPrefs.hitsoundVolume = 0; //I'm tired of hitsounds in playstate, like wtf.
		...
		menuscripts.executeAllFunc("startIntroPost", []);
	...
	public function selectionControls(expected:String):Bool {
	...
	function _selectionControls(expected:String){
		
		function pressedEnter(){
			var didPressEnter:Bool = FlxG.keys.justPressed.ENTER || controls.ACCEPT;
			#if mobile
	...
	override function update(elapsed:Float)
	{
		...
		//var pressedEnter:Bool = selectionControls("pressedEnter");
		...
		var toGame = selectionControls("pressedEnterToGame");
		...
		if (initialized && selectionControls("pressedEnterToSkip") && !skippedIntro)
		{
			skipIntro();
		}
		...
		if(swagShader != null)
		{
			if(selectionControls("UI_LEFT")) swagShader.hue -= elapsed * 0.1;
			if(selectionControls("UI_RIGHT")) swagShader.hue += elapsed * 0.1;
		}
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = -20)
	{
		for (i in 0...textArray.length)
		{
			var money:Alphabet = new Alphabet(0, 0, textArray[i], true);
			money.screenCenter(X);
			money.y += (i * Alphabet.Y_PER_ROW_BOLD) + 200 + Alphabet.boldLetterAdditions[1] + offset-20;
	...
	function addMoreText(text:String, ?offset:Float = -20)
	{
		if(textGroup != null && credGroup != null) {
			var coolText:Alphabet = new Alphabet(0, 0, text, true);
			coolText.screenCenter(X);
			coolText.y += (textGroup.length * Alphabet.Y_PER_ROW_BOLD) + 200 + Alphabet.boldLetterAdditions[1] + offset -20;
