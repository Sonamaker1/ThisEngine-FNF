...
class Paths
{
	...
	inline static public function voices(song:String):Any
	{
		...
		var diff:String = CoolUtil.difficulties[PlayState.storyDifficulty].toUpperCase();
		var exactMatch:Bool = false;
		if(!FileSystem.exists(songKey+".ogg")){
			var possiblyHere:String = Paths.mods(Paths.currentModDirectory + '/songs/' + Paths.formatToSongPath(song) + '/');
			var pathsFiles:Array<String> = CoolUtil.findFilesInPath(possiblyHere,["ogg"]);
			for (_ in pathsFiles){
				if(_.endsWith('Voices.ogg')){
					if(_.endsWith('${diff}Voices.ogg')){
						trace("VOICES_EXACTMATCH: "+_);
						exactMatch = true;
						songKey = '${formatToSongPath(song)}/${_}'.replace('.ogg','');
					}else if(!exactMatch){
						trace("VOICES_TRYING: "+_);
						songKey = '${formatToSongPath(song)}/${_}'.replace('.ogg','');
					}
				}
			}
	...
	inline static public function inst(song:String):Any
	{
		...
		if(!FileSystem.exists(songKey+".ogg")){
			...
			var pathsFiles:Array<String> = CoolUtil.findFilesInPath(possiblyHere,["ogg"]);
			for (_ in pathsFiles){
				if(_.endsWith('Inst.ogg')){
					if(_.endsWith('${diff}Inst.ogg')){
						trace("INST_EXACTMATCH: "+_);
						exactMatch = true;
						songKey = '${formatToSongPath(song)}/${_}'.replace('.ogg','');
					}else if(!exactMatch){
						trace("INST_Trying: "+_);
