...
#if VIDEOS_ALLOWED
#if (hxCodec == "3.0.2") import hxcodec.flixel.FlxVideo;
...
class PlayState extends MusicBeatState
{
	...
	public function callOnLuas(event:String, args:Array<Dynamic>, ignoreStops = true, exclusions:Array<String> = null):Dynamic {
		var isPauseHscript:Bool = ScriptUtil.hasPause(scripts.executeAllFunc(event, args));
		var returnVal:Dynamic = FunkinLua.Function_Continue;
		#if LUA_ALLOWED
		if(exclusions == null) exclusions = [];
		if(isPauseHscript)
		{
			returnVal = isPauseHscript && !ignoreStops;
		}
		else{
			for (script in luaArray) {
	...
	function initScripts()
	{
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				for (_ in ScriptUtil.findScriptsInDir(folder))
					files.push(_);
			}
		}
		...
		if (SONG.stage != null)
		{
			...
			{
				var modspath:String = Paths.mods(Paths.currentModDirectory + '/stages/' + SONG.stage + '.$extn');
				var path:String = Paths.getPreloadPath('stages/' + SONG.stage + '.$extn');
				if (FileSystem.exists(modspath)){
					trace("FOUND:"+modspath);
					hx = File.getContent(modspath);
					break;
				}
